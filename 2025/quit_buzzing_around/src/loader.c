...
#include <sys/types.h> // Added for stat
...
int main(int argc, char **argv)
{
	...
	memset(skel->data->g, 0, 32);

	// Get current mount namespace inode and set it in BPF program
	struct stat st;
	if (stat("/proc/self/ns/mnt", &st) != 0) {
		dbg("Failed to stat /proc/self/ns/mnt\n");
		goto cleanup;
	}
	dbg("Detected Mount NS: %lu. Setting filter.\n", st.st_ino);
	skel->bss->k = st.st_ino;
	...
	int fd = bpf_map__fd(skel->maps.d);
